...
function executeCallback (id, error, value) {
  RamaPay.executeCallback(id, error, value)
}

window.RamaPay.init(__rpcURL, {
  getAccounts: function (cb) { cb(null, [__addressHex]) },
  processTransaction: function (tx, cb){
    console.log('signing a transaction', tx)
    const { id = 8888 } = tx
    RamaPay.addCallback(id, cb)

    var gasLimit = tx.gasLimit || tx.gas || null;
    var gasPrice = tx.gasPrice || null;
    var data = tx.data || null;
    var nonce = tx.nonce || -1;
    ramapay.signTransaction(id, tx.to || null, tx.value, nonce, gasLimit, gasPrice, data);
  },
  ...
  signTypedMessage: function (msgParams, cb) {
    ...
    const { id = 8888 } = msgParams
    RamaPay.addCallback(id, cb)
    ramapay.signTypedMessage(id, JSON.stringify(msgParams))
  },
  ethCall: function (msgParams, cb) {
    ...
    RamaPay.addCallback(id, cb)
    ramapay.ethCall(id, JSON.stringify(msgParams));
    //ramapay.ethCall(id, msgParams.to, msgParams.data, msgParams.value);
  },
  walletAddEthereumChain: function (msgParams, cb) {
    ...
    RamaPay.addCallback(id, cb)
    ramapay.walletAddEthereumChain(id, JSON.stringify(msgParams));
  ...
  walletSwitchEthereumChain: function (msgParams, cb) {
    ...
    RamaPay.addCallback(id, cb)
    ramapay.walletSwitchEthereumChain(id, JSON.stringify(msgParams));
...
window.web3.setProvider = function () {
  console.debug('RamaPay - overrode web3.setProvider')
}
